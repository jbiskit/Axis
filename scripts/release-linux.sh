#!/usr/bin/env bash
# Local Linux release prep/build for Axis.
# Windows CI still publishes axis.exe via .github/workflows/release.yml after --publish.
set -euo pipefail

usage() {
  cat <<'EOF'
Usage: ./scripts/release-linux.sh <version> [options]

Linux counterpart to scripts/release.sh. Builds target/release/axis (no .exe).

Arguments:
  version          Next release version, for example 0.1.6 or v0.1.6
...
Options:
  ...
  --publish        Commit and push; GitHub Actions creates the tag and Windows release
...
if [[ "$(uname -s)" != "Linux" ]]; then
  printf '%s\n' \
    'scripts/release-linux.sh is for Linux hosts only.' \
    'On Windows use scripts/release.sh (or npm run release if configured).' >&2
  exit 1
fi
...
if [[ ! "$version" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
  printf 'A semantic version such as 0.1.6 is required.\n\n' >&2
  usage >&2
  exit 2
fi
...
if ! command -v cargo-tauri >/dev/null 2>&1 && ! cargo tauri --version >/dev/null 2>&1; then
  printf '%s\n' \
    'Tauri CLI 2 is required (cargo tauri).' \
    'Install with: cargo install tauri-cli --version "^2"' >&2
  exit 1
fi
...
printf 'Bumping Axis %s -> %s (Linux release build)\n' "$current_version" "$version"
...
if [[ "$version_only" == false ]]; then
  ...
  printf '\nBuilding portable Linux Axis binary...\n'
  (
    cd "$tauri_directory"
    cargo tauri build
  )

  executable="$repository_root/target/release/axis"
  if [[ ! -f "$executable" ]]; then
    printf "Build completed but '%s' was not found.\n" "$executable" >&2
    printf '%s\n' \
      'On Linux the portable binary is target/release/axis (no .exe).' \
      'If Tauri wrote elsewhere, check target/release/ after the build.' >&2
    false
  fi
...
if [[ "$publish" == true ]]; then
  ...
  printf '\nPushed release: %s. GitHub Actions will create v%s and publish the Windows axis.exe.\n' \
    "$version" "$version"
  exit 0
fi

printf '\nVersion %s is prepared (Linux binary verified when not --version-only).\n' "$version"
printf '%s\n' \
  'Review with: git diff -- Cargo.toml Cargo.lock crates/axis-tauri' \
  'This script does not commit, tag, push, or publish unless you pass --publish.'
